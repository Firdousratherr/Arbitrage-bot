...
async def test_lbank_uses_exchange_market_id_for_book_ticker():
    ...
    _base_adapter_state(adapter, 'lbank', {
        'BTC/USDT': {'id': 'btc_usdt', 'active': True, 'spot': True, 'type': 'spot'},
        'ETH/USDT': {'id': 'eth_usdt', 'active': True, 'spot': True, 'type': 'spot'},
    })
    calls = []
    ...
    adapter.client = SimpleNamespace(spotPublicGetSupplementTickerBookTicker=fake_book_ticker)
    ...
    assert result[0].quote_volume == 0.0
    assert calls == ['btc_usdt']


@pytest.mark.asyncio
async def test_lbank_enriches_volume_with_one_bulk_request():
    adapter = object.__new__(LBankAdapter)
    _base_adapter_state(adapter, 'lbank', {
        'BTC/USDT': {'id': 'btc_usdt', 'active': True, 'spot': True, 'type': 'spot'},
    })
    ...
    async def fake_book_ticker(params):
        return {'data': {'bidPrice': '100.0', 'askPrice': '101.0'}, 'ts': 1700000000000}

    bulk = AsyncMock(return_value={
        'BTC/USDT': {'quoteVolume': '25000'},
    })
    ...
    assert result[0].quote_volume == 25000.0
    assert adapter.last_ticker_source == 'book_ticker+bulk_volume'
    bulk.assert_awaited_once_with(['BTC/USDT'])
...
async def test_xt_uses_batch_best_bid_ask_endpoint():
    ...
    async def fake_bids_asks(symbols):
        ...
        return {'BTC/USDT': {'symbol': 'BTC/USDT', 'bid': 100.0, 'ask': 101.0, 'timestamp': 1700000000000}}
...
async def test_generic_ccxt_uses_fetch_bids_asks_when_supported():
    ...
    _base_adapter_state(adapter, 'mexc', {'BTC/USDT': {'active': True, 'spot': True, 'type': 'spot'}})
    adapter.last_ticker_source = ''

    adapter.client = SimpleNamespace(
        has={'fetchBidsAsks': True},
        fetch_bids_asks=AsyncMock(return_value={'BTC/USDT': {'bid': '100.0', 'ask': '101.0', 'quoteVolume': '50000', 'timestamp': 1700000000000}}),
    )
...
async def test_generic_ccxt_falls_back_to_symbol_scoped_fetch_tickers():
    adapter = object.__new__(CcxtAdapter)
    _base_adapter_state(adapter, 'kraken', {'BTC/USDT': {'active': True, 'spot': True, 'type': 'spot'}})
    adapter.last_ticker_source = ''

    adapter.client = SimpleNamespace(
        has={'fetchBidsAsks': False},
        fetch_tickers=AsyncMock(return_value={'BTC/USDT': {'bid': '100.0', 'ask': '101.0', 'quoteVolume': '50000', 'timestamp': 1700000000000}}),
    )
