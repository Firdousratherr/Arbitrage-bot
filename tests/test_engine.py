...
def test_mismatched_explicit_asset_identity_is_deferred_to_transfer_validation():
    o=pair_opportunity(t('a',100,'BTC|contract:0xaaa'),t('b',102,'BTC|contract:0xbbb'),.1,.1)
    assert o is not None
    assert o.metadata['asset_identity_conflict'] is True

def test_identity_present_on_only_one_side_does_not_hide_price_candidate():
    o=pair_opportunity(t('a',100,'BTC|contract:0xaaa'),t('b',102),.1,.1)
    assert o is not None
