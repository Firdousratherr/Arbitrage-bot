...
Optional AI maintenance support uses an OpenAI-compatible endpoint. Set `AI_API_URL`, `AI_API_KEY`, `AI_MODEL`, `AI_MAX_INPUT_TOKENS=5500`, and `MAINTENANCE_REPO_PATH` to enable `/diagnose` and `/fixerror`; `AI_FALLBACK_MODEL` is optional. These commands are admin-only. The assistant can discover and search the complete repository, including application code, tests, configuration, deployment files, documentation, and relevant logs, then reads relevant file sections and sends only ranked, redacted evidence within the token budget. It never reads `.env` files, credentials, private keys, or database contents. In the AWS service, the host repository is mounted at `/maintenance/repo` without Docker socket access. `/fixerror` creates a proposal; `/validatefix PATCH_ID` runs `git apply --check`, syntax checks, and tests; `/rejectfix PATCH_ID` rejects it; and `/approvefix PATCH_ID` applies it only after explicit admin approval. Patches cannot modify `.env`, credentials, `.git`, logs, data, private keys, or database files; source, tests, Docker, deployment, and documentation files may be proposed when necessary. Failed validation, tests, or post-apply health checks trigger automatic reverse rollback. Restarting or rebuilding the Docker image remains an explicit deployment operation reported to the administrator.
