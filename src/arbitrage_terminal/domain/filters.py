...
class ScanFilters:
    ...
    def check(self, o: Opportunity, include_validation: bool = True):
        ...
        if o.data_age_seconds > self.max_data_age:
            return f'data age {o.data_age_seconds:.1f}s exceeds limit'
        if self.selected_coins and o.symbol.split('/', 1)[0].upper() not in self.selected_coins:
            return 'coin not selected'
        if self.quote_currency and o.symbol.split('/', 1)[1].upper() != self.quote_currency:
