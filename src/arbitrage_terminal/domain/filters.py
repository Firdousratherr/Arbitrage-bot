...
class ScanFilters:
    min_gap:float=.50; min_net_profit:float=.20; min_volume:float=10000.; min_liquidity:float=1000.; max_data_age:float=10.
    require_network:bool=True; require_fees:bool=False; selected_coins:set[str]=field(default_factory=set); quote_currency:str='USDT'; validation_mode:str='strict'
    def check(self,o:Opportunity,include_validation:bool=True):
        ...
        if o.estimated_net_profit is None:return 'net profit unavailable because required fee data is missing'
        ...
        if include_validation and self.validation_mode.lower()!='loose':
