...
class CcxtAdapter(ExchangeAdapter):
    ...
    def _new_client(self):
        return self._client_class({'enableRateLimit': True, 'timeout': 15000, **self._credentials})
    ...
    async def repair(self):
        """Recreate a poisoned CCXT client and discard stale market/currency caches."""
        old = self.client
        self._markets = {}
        self._currencies = None
        self._currencies_task = None
    ...
    def _asset_identity(base, market, currency):
        addresses = set()
        def add(value):
            if value is not None and str(value).strip(): addresses.add(str(value).strip().lower())
        for source in (currency or {}, market or {}, (currency or {}).get('info') or {}, (market or {}).get('info') or {}):
            if isinstance(source, dict):
                for key in ('contractAddress', 'contract_address', 'tokenAddress', 'token_address', 'address'): add(source.get(key))
        ...
        for _, network in entries:
            ...
            for field in ('contractAddress', 'contract_address', 'tokenAddress', 'token_address', 'address'): add(network.get(field))
        if not addresses: return None
        return f"{base.upper()}|contract:{'|'.join(sorted(addresses))}"

    async def get_markets(self):
        self.last_market_symbols = set(); self.last_market_asset_identities = {}; self.last_ticker_symbols = set(); self.last_ticker_count = 0; self.last_ticker_source = ''
        ...
        if isinstance(currencies, dict): self._currencies = currencies
        for raw, m in self._markets.items():
            ...
            if identity: self.last_market_asset_identities[sym] = identity
    ...
    def _parse_tickers(self, name, raw, wanted):
        ...
        for raw_symbol, t in (raw or {}).items():
            ...
            ts = t.get('timestamp'); stamp = datetime.fromtimestamp(float(ts) / 1000, timezone.utc) if ts else datetime.now(timezone.utc)
    ...
    async def _get_currencies(self):
        ...
        if isinstance(cached, dict) and cached: self._currencies = cached; return cached
        return {}
    ...
    async def get_transfer_info(self, asset):
        ...
        for key, n in (info.get('networks') or {}).items():
            n = n or {}; address = n.get('contractAddress') or n.get('contract_address') or n.get('address')
            networks.append({'network': key, 'deposit': n.get('deposit') is not False, 'withdraw': n.get('withdraw') is not False, 'fee': n.get('fee'), 'contract_address': address})
