...
class SelfHealingAdapter:
    """Transparent adapter supervisor with persistent learned recovery strategies."""
    ...
    async def _perform_deterministic_recovery(self, action: str = "repair") -> bool:
        self.health.state = "recovering"
        self.health.last_recovery_action = action
        if action == "retry":
            try:
    ...
    async def _ai_recover(self, original_error: Exception) -> bool:
        ...
        try:
            ...
            if action == "retry": return await self._perform_deterministic_recovery("retry")
            if action in {"repair", "reload_markets", "invalidate_cache"}: return await self._perform_deterministic_recovery(action)
            return False
        except Exception:
    ...
    async def _recover(self, original_error: Exception | None = None) -> bool:
        async with self._recovery_lock:
            if self.health.state == "healthy" and (original_error is None or self.health.consecutive_failures == 0):
                return True
            ...
            if preferred:
                ...
                if ok:
                    try: await self.recovery_memory.record(self.name, error_type, preferred, True)
                    except Exception: pass
                    return True
                try: await self.recovery_memory.record(self.name, error_type, preferred, False)
                except Exception: pass
            deterministic_ok = await self._perform_deterministic_recovery("repair")
            if deterministic_ok:
                if self.recovery_memory is not None:
                    try: await self.recovery_memory.record(self.name, error_type, "repair", True)
                    except Exception: pass
                return True
            if self.recovery_memory is not None:
                try: await self.recovery_memory.record(self.name, error_type, "repair", False)
                except Exception: pass
    ...
    async def repair(self) -> bool:
        async with self._recovery_lock:
            return await self._perform_deterministic_recovery("repair")
...
class ExchangeSelfHealingSupervisor:
    def __init__(self, adapters: dict[str, Any], failure_threshold: int = 3, quarantine_seconds: float = 30.0, recovery_advisor: Any = None, recovery_memory: Any = None):
