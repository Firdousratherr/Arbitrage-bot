...
from .bot.contact import build_contact_conversation, contact_cancel
...
async def _run():
    ...
    app.add_handler(build_contact_conversation())
    ...
    app.add_handler(CallbackQueryHandler(filters_callback,pattern=r'^filters$')); app.add_handler(CallbackQueryHandler(filter_settings_callback,pattern=r'^filter:')); app.add_handler(CallbackQueryHandler(ai_workbench_callback,pattern=r'^aiwb:')); app.add_handler(CallbackQueryHandler(ai_recovery_callback,pattern=r'^ai_recovery:')); app.add_handler(CallbackQueryHandler(ai_settings_callback,pattern=r'^settings$')); app.add_handler(CallbackQueryHandler(admin_callback,pattern=r'^admin:'))
    ...
    app.add_handler(CommandHandler('cancel',contact_cancel)); app.add_handler(CommandHandler('airecovery',ai_recovery_cmd))
    ...
    try: await asyncio.Event().wait()
        ...
        await app.updater.stop(); await app.stop(); await app.shutdown(); await service.close(); await asyncio.gather(*(x.close() for x in exchanges.values()),return_exceptions=True); await ai.close(); await repo.close()
