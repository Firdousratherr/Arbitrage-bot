...
class ArbitrageScanner:
    ...
    async def _load_markets_one(self, name, adapter, budget):
        ...
        try:
            async with self.semaphore:
                markets, retries = await self._call(adapter.get_markets, time.monotonic()+budget)
            ...
            breaker.failure(); diag.status='failed'; diag.latency_ms=(time.perf_counter()-started)*1000
            diag.timestamp=datetime.now(timezone.utc).isoformat(); diag.error_type=getattr(exc,'error_type',type(exc).__name__)
            diag.http_status=getattr(exc,'http_status',None); diag.detail=str(exc)[:500]
    ...
    async def _load_tickers_one(self, name, adapter, symbols, budget):
        ...
        try:
            async with self.semaphore:
                tickers,retries=await self._call(lambda: adapter.get_tickers(symbols), time.monotonic()+budget)
    ...
    async def scan(self, user_id, selected, filters, progress=None):
        ...
        selected=list(dict.fromkeys(str(x).lower() for x in selected)); missing=[n for n in selected if n not in self.exchanges]
        ...
        waves=max(1,math.ceil(len(adapters)/self.concurrency)); exchange_budget=max(5.0,min(self.timeout*2.0,(self.scan_deadline-2.0)/waves))

        async def load_market(name,adapter):
            await emit('exchange',exchange=name,status='loading',markets=0,tickers=0)
            result=await self._load_markets_one(name,adapter,exchange_budget)
            await emit('exchange',exchange=name,status='healthy' if result[3] is None else 'failed',markets=len(result[1]),tickers=0)
            return result

        try:
            market_results=await asyncio.wait_for(asyncio.gather(*(load_market(n,a) for n,a in adapters.items())),self.scan_deadline)
            ...
            await emit('complete',comparisons=0,opportunities=0,healthy=0,failed=len(selected))
            return ScanSnapshot(scan_id,user_id,started,datetime.now(timezone.utc).isoformat(),selected,[],[],selected,0,0,0,0,ScanState.FAILED,diagnostics=diagnostics,warnings=[f'Scan stopped after the {self.scan_deadline:.0f}s safety deadline.'],errors=['Scan timed out before market data collection completed.'])
        ...
        market_sets={r[0]:r[1] for r in market_results}; union=set().union(*(market_sets.values())) if market_sets else set()
        counts={}
        for symbols in market_sets.values():
            for symbol in symbols: counts[symbol]=counts.get(symbol,0)+1
        shared={s for s,c in counts.items() if c>=2}; symbols_by_exchange={n:market_sets.get(n,set())&shared for n in healthy}
        await emit('markets',healthy=len(healthy),failed=len(failed),markets=len(union),symbols=len(shared))

        remaining=max(1.0,self.scan_deadline-(time.monotonic()-started_mono)); ticker_budgets={}
        for n in healthy:
            count=len(symbols_by_exchange.get(n,set()))
            ticker_budgets[n]=min(remaining,max(exchange_budget,min(90.0,count/7.0+8.0))) if n=='lbank' and count else min(remaining,exchange_budget)
        if healthy and time.monotonic()-started_mono<self.scan_deadline:
            try:
                ticker_results=await asyncio.wait_for(asyncio.gather(*(self._load_tickers_one(n,adapters[n],symbols_by_exchange[n],ticker_budgets[n]) for n in healthy)),max(1.0,self.scan_deadline-(time.monotonic()-started_mono)))
            except asyncio.TimeoutError:
                ticker_results=[(n,[],0,0.0,asyncio.TimeoutError('Ticker collection exceeded scan deadline.')) for n in healthy]
        else:
            ticker_results=[(n,[],0,0.0,asyncio.TimeoutError('Scan deadline reached before ticker collection.')) for n in healthy]
        ...
        for n,tickers,retries,latency,error in ticker_results:
            ...
            for ticker in tickers: ticker_map.setdefault(ticker.symbol,[]).append(ticker)
            for diag in diagnostics:
                if diag.exchange==n and diag.operation=='market_data': diag.retry_count+=retries; diag.latency_ms+=latency; break
        ...
        rejected=[]; comparisons=0; fee_maps={}; degraded=[]; opportunities=[]
        ...
        remaining=max(1.0,self.scan_deadline-(time.monotonic()-started_mono))
        if healthy and ticker_map and time.monotonic()-started_mono<self.scan_deadline:
            try: fee_results=await asyncio.wait_for(asyncio.gather(*(fee_one(n,adapters[n]) for n in healthy)),remaining)
        ...
        for n,data,error in fee_results:
            fee_maps[n]=data
            if error:
                degraded.append(n); diagnostics.append(Diagnostic(n,'trading_fees','degraded',0,datetime.now(timezone.utc).isoformat(),error_type=getattr(error,'error_type',type(error).__name__),http_status=getattr(error,'http_status',None),detail=str(error)[:500])); warnings.append(f'{n}: trading fee data unavailable; affected opportunities have unknown net profit.')
        await emit('fees',completed=len(healthy),degraded=len(degraded))

        orderbook_cache={}; orderbook_tasks={}; ob_sem=asyncio.Semaphore(self.orderbook_concurrency)
        async def get_orderbook(exchange,symbol):
            ...
            if key not in orderbook_tasks:
                async def load():
                    async with ob_sem:
                        remaining=self.scan_deadline-(time.monotonic()-started_mono)
                        if remaining<=0:return TimeoutError('Order-book validation deadline reached.')
                        try: book,_=await self._call(lambda:adapters[exchange].get_orderbook(symbol,limit=20),time.monotonic()+min(self.timeout,remaining)); return book
                        except Exception as exc:return exc
                orderbook_tasks[key]=asyncio.create_task(load())
            task=orderbook_tasks[key]; remaining=self.scan_deadline-(time.monotonic()-started_mono)
            if remaining<=0:
                task.cancel(); orderbook_tasks.pop(key,None); return TimeoutError('Order-book validation deadline reached.')
            try: result=await asyncio.wait_for(task,min(self.timeout,remaining))
            except asyncio.TimeoutError:
                task.cancel(); orderbook_tasks.pop(key,None); result=TimeoutError(f'Order-book deadline reached for {exchange} {symbol}.')
            orderbook_cache[key]=result if isinstance(result,Exception) else (result or {})
        ...
        async def validate_depth(o):
            ...
            net=None if o.buy_fee is None or o.sell_fee is None else gap-float(o.buy_fee)-float(o.sell_fee)-float(o.withdrawal_cost or 0.0)
            meta=dict(o.metadata); meta.update({'orderbook_validated':True,'orderbook_depth_limit':20,'executable_gap_pct':gap,'ticker_gap_pct':o.raw_gap,'buy_average_price':float(result['buy_average_price']),'sell_average_price':float(result['sell_average_price']),'executable_quote_size':float(result['quote_spent']),'executable_base_amount':float(result['base_acquired']),'executable_quote_received':float(result['quote_received']),'estimated_slippage_pct':o.raw_gap-gap})
            return replace(o,buy_price=float(result['buy_average_price']),sell_price=float(result['sell_average_price']),raw_gap=gap,estimated_net_profit=net,metadata=meta),None

        await emit('candidates',message='Evaluating price gaps before order-book/network validation')
        timed_out=False; depth_validated=[]; candidate_batch=[]; depth_completed=0; depth_started=time.monotonic(); batch_size=max(2,self.orderbook_concurrency*2)

        async def flush_depth_batch():
            nonlocal depth_completed,timed_out
            if not candidate_batch:return
            batch=list(candidate_batch); candidate_batch.clear()
            await emit('orderbook',comparisons=comparisons,opportunities=len(depth_validated),validated=depth_completed,total=depth_completed+len(batch),message=f'Validating executable order books · {depth_completed}/{depth_completed+len(batch)}')
            async def one(o):
                remaining_budget=self.orderbook_budget-(time.monotonic()-depth_started); remaining_scan=self.scan_deadline-(time.monotonic()-started_mono)
                if remaining_budget<=0 or remaining_scan<=0:return o,None,'order-book validation budget exhausted'
                try:return o,*(await asyncio.wait_for(validate_depth(o),min(self.timeout,remaining_budget,remaining_scan)))
        ...
        for symbol,tickers in ticker_map.items():
            ...
            for buy in valid:
                for sell in valid:
                    ...
                    if time.monotonic()-started_mono>=self.scan_deadline:timed_out=True;break
                    comparisons+=1
                    buy_fee=fee_maps.get(buy.exchange,{}).get(symbol); sell_fee=fee_maps.get(sell.exchange,{}).get(symbol)
                    opportunity=pair_opportunity(buy,sell,buy_fee,sell_fee,None,filters.max_data_age,{'network_available':False,'contract_match':False,'networks':[]})
                    if opportunity:
                        reason=filters.check(opportunity,include_validation=False)
                        if reason:rejected.append({'symbol':symbol,'buy':buy.exchange,'sell':sell.exchange,'reason':reason})
                            ...
                            if time.monotonic()-depth_started>self.orderbook_budget:rejected.append({'symbol':symbol,'buy':buy.exchange,'sell':sell.exchange,'reason':'order-book validation budget exhausted'})
                                ...
                                candidate_batch.append(opportunity)
                                if len(candidate_batch)>=batch_size:await flush_depth_batch()
                        else:depth_validated.append(opportunity)
                    if comparisons<=2 or comparisons%25==0:
                        await emit('candidates',comparisons=comparisons,opportunities=len(depth_validated),symbols_processed=processed_symbols,message='Comparing exchange prices…')
                    if comparisons%25==0:await asyncio.sleep(0)
                if timed_out:break
            if processed_symbols%25==0:
                await emit('candidates',comparisons=comparisons,opportunities=len(depth_validated),symbols_processed=processed_symbols,message='Comparing exchange prices…'); await asyncio.sleep(0)
            if timed_out:break

        if candidate_batch and not timed_out:await flush_depth_batch()
        ...
        async def get_transfer(exchange,asset):
            ...
            task=transfer_tasks[key]; remaining=self.scan_deadline-(time.monotonic()-started_mono)
            if remaining<=0:task.cancel();transfer_tasks.pop(key,None);return {}
            try:result=await asyncio.wait_for(task,min(self.timeout,remaining))
            ...
            if isinstance(result,Exception):
                diagnostics.append(Diagnostic(exchange,'transfer_info','degraded',0,datetime.now(timezone.utc).isoformat(),error_type=getattr(result,'error_type',type(result).__name__),detail=f'{asset}: {str(result)[:350]}'));warnings.append(f'{exchange}: transfer information unavailable for {asset}; strict validation rejected affected routes.');degraded.append(exchange);result={}
            transfer_cache[key]=result or {};return transfer_cache[key]
        ...
        if timed_out:warnings.append(f'Scan stopped at the {self.scan_deadline:.0f}s safety deadline; results are partial.')
            ...
            budget=min(self.scan_deadline-(time.monotonic()-started_mono),self.network_validation_budget)
            ...
            for index,o in enumerate(opportunities,1):
                ...
                network,contract,networks=transfer_compatibility(bn,sn); base_amount=o.metadata.get('executable_base_amount')
                if base_amount is None:
                    try:base_amount=filters.trade_size/float(o.buy_price)
                    except (TypeError,ValueError,ZeroDivisionError):base_amount=None
                withdrawal_pct=withdrawal_cost_pct(bn,networks,base_amount) if network and contract else None
                ...
                if network and contract and withdrawal_pct is not None:
                    ...
                    verified=replace(o,withdrawal_cost=withdrawal_pct,estimated_net_profit=net,metadata={**o.metadata,**transfer,'withdrawal_fee_available':True,'withdrawal_fee_pct':withdrawal_pct,'withdrawal_fee_networks':networks,'withdrawal_fee_base_asset':asset,'withdrawal_fee_base_amount':base_amount},confidence=min(100.0,o.confidence+5.0))
                ...
                if index%25==0 or index==total:await emit('network',comparisons=comparisons,opportunities=len(validated),validated=index,total=total,message='Validating transfer networks…');await asyncio.sleep(0)
