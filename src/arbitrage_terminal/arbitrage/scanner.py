...
class ArbitrageScanner:
    ...
    async def _call(self, fn, deadline=None):
        ...
        for attempt in range(3):
            try:
                ...
                if remaining is not None and remaining <= sleep_for:
                    raise asyncio.TimeoutError
    ...
    async def scan(self, user_id, selected, filters, progress=None):
        ...
        ticker_budgets = {}
        remaining_scan = max(1.0, self.scan_deadline - (time.monotonic() - started_mono))
        for name in healthy:
            count = len(symbols_by_exchange.get(name, set()))
            if name == 'lbank' and count:
                estimated = count / 7.0 + 8.0
        ...
        orderbook_cache = {}
        orderbook_tasks = {}
        orderbook_semaphore = asyncio.Semaphore(self.orderbook_concurrency)
            ...
            if key not in orderbook_tasks:
                async def load():
                    async with orderbook_semaphore:
                        ...
                        if remaining_scan <= 0:
                            return TimeoutError('Order-book validation deadline reached.')
                        try:
                            book, _ = await self._call(lambda: adapters[exchange].get_orderbook(symbol, limit=20), time.monotonic() + min(self.timeout, remaining_scan))
                            return book
                        except Exception as e:
                            return e
                orderbook_tasks[key] = asyncio.create_task(load())
            task = orderbook_tasks[key]
            remaining_scan = self.scan_deadline - (time.monotonic() - started_mono)
            if remaining_scan <= 0:
                task.cancel()
                orderbook_tasks.pop(key, None)
                return TimeoutError('Order-book validation deadline reached.')
            try:
                ...
                task.cancel()
                orderbook_tasks.pop(key, None)
                result = TimeoutError(f'Order-book deadline reached for {exchange} {symbol}.')
            if isinstance(result, Exception):
                orderbook_cache[key] = result
            else:
                orderbook_cache[key] = result or {}
            return orderbook_cache[key]
        ...
        async def validate_depth(o):
            ...
            if isinstance(buy_book, Exception) or isinstance(sell_book, Exception):
                err = buy_book if isinstance(buy_book, Exception) else sell_book
                return None, f'order-book unavailable: {str(err)[:250]}'
            ...
            if gap <= 0:
                return None, f'executable spread {gap:.3f}% is non-positive'
            net = None
            ...
            buy_avg = float(result['buy_average_price'])
            sell_avg = float(result['sell_average_price'])
        ...
        candidate_batch = []
        depth_completed = 0
        batch_size = max(2, self.orderbook_concurrency * 2)
        ...
        async def flush_depth_batch():
            ...
            async def validate_one(o):
                if time.monotonic() - started_mono >= self.scan_deadline:
                    return o, None, 'order-book validation deadline reached'
                remaining_budget = self.orderbook_budget - (time.monotonic() - depth_started)
                remaining_scan = self.scan_deadline - (time.monotonic() - started_mono)
                if remaining_budget <= 0 or remaining_scan <= 0:
                    return o, None, 'order-book validation budget exhausted'
                try:
                    checked, reason = await asyncio.wait_for(validate_depth(o), timeout=min(self.timeout, remaining_budget, remaining_scan))
                    return o, checked, reason
                except asyncio.TimeoutError:
                    return o, None, 'order-book validation timed out'
                except Exception as exc:
                    return o, None, f'order-book validation failed: {str(exc)[:250]}'

            tasks = [asyncio.create_task(validate_one(o)) for o in batch]
            try:
                for task in asyncio.as_completed(tasks):
                    original, checked, reason = await task
                    depth_completed += 1
                    if reason:
                        rejected.append({'symbol': original.symbol, 'buy': original.buy_exchange, 'sell': original.sell_exchange, 'reason': reason})
                    elif checked is not None:
                        reason = filters.check(checked, include_validation=False)
                        if reason:
                            rejected.append({'symbol': checked.symbol, 'buy': checked.buy_exchange, 'sell': checked.sell_exchange, 'reason': reason})
                        else:
                            depth_validated.append(checked)
                    await emit('orderbook', comparisons=comparisons, opportunities=len(depth_validated), validated=depth_completed, total=depth_completed + len(tasks) - depth_completed, message=f'Validating executable order books · {depth_completed}/{len(tasks)} complete')
            finally:
                for task in tasks:
                    if not task.done():
                        task.cancel()
        ...
        for symbol, tickers in ticker_map.items():
            ...
            for buy in valid:
                for sell in valid:
                    ...
                    if not o:
                        if comparisons % 10 == 0 or comparisons <= 2:
                            await emit('candidates', comparisons=comparisons, opportunities=len(depth_validated), message='Comparing exchange prices…')
                        continue
                    ...
                    if reason:
                        ...
                        if time.monotonic() - depth_started > self.orderbook_budget:
                            rejected.append({'symbol': symbol, 'buy': buy.exchange, 'sell': sell.exchange, 'reason': 'order-book validation budget exhausted'})
                        else:
                            candidate_batch.append(o)
        ...
        if candidate_batch and not timed_out:
            await flush_depth_batch()
        ...
        if timed_out:
            ...
            for index, o in enumerate(opportunities, 1):
                ...
                bn = transfer_cache.get((o.buy_exchange, asset), {})
                sn = transfer_cache.get((o.sell_exchange, asset), {})
                ...
                if network and contract:
                    ...
                    if base_amount is None:
                        try:
                            base_amount = filters.trade_size / float(o.buy_price)
