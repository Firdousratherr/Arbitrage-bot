...
class ArbitrageScanner:
    ...
    async def _load_markets_one(self, name, adapter, exchange_budget):
        ...
        if not breaker.available:
            ...
            diag.status = 'failed'; diag.error_type = e.error_type; diag.detail = str(e)
        ...
        try:
            ...
            diag.status = 'ok'; diag.retry_count = retries
            diag.latency_ms = (time.perf_counter() - started) * 1000
            diag.timestamp = datetime.now(timezone.utc).isoformat(); breaker.success()
            return name, symbols, diag, None
        except Exception as e:
            breaker.failure(); diag.status = 'failed'
            diag.latency_ms = (time.perf_counter() - started) * 1000
            diag.timestamp = datetime.now(timezone.utc).isoformat()
            diag.error_type = getattr(e, 'error_type', type(e).__name__)
            diag.http_status = getattr(e, 'http_status', None); diag.detail = str(e)[:500]
            return name, set(), diag, e
    ...
    async def scan(self, user_id, selected, filters, progress=None):
        ...
        started = datetime.now(timezone.utc).isoformat(); started_mono = time.monotonic()
        ...
        for name, tickers, retries, latency_ms, error in ticker_results:
            ...
            for diag in diagnostics:
                if diag.exchange == name and diag.operation == 'market_data':
                    diag.retry_count += retries; diag.latency_ms += latency_ms; break
        ...
        for n, data, e in fee_results:
            fee_maps[n] = data
            if e:
                degraded.append(n); diagnostics.append(Diagnostic(n, 'trading_fees', 'degraded', 0, datetime.now(timezone.utc).isoformat(), error_type=getattr(e, 'error_type', type(e).__name__), http_status=getattr(e, 'http_status', None), detail=str(e)[:500]))
        ...
        for symbol, tickers in ticker_map.items():
            ...
            for buy in valid:
                for sell in valid:
                    ...
                    if reason:
                        rejected.append({'symbol': symbol, 'buy': buy.exchange, 'sell': sell.exchange, 'reason': reason}); continue
                    if filters.require_orderbook and filters.validation_mode != 'loose':
                        if time.monotonic() - depth_started > self.orderbook_budget:
                            rejected.append({'symbol': symbol, 'buy': buy.exchange, 'sell': sell.exchange, 'reason': 'order-book validation budget exhausted'}); continue
                        ...
                        if depth_reason:
                            rejected.append({'symbol': symbol, 'buy': buy.exchange, 'sell': sell.exchange, 'reason': depth_reason}); continue
                        o = checked
                        reason = filters.check(o, include_validation=False)
                        if reason:
                            rejected.append({'symbol': symbol, 'buy': buy.exchange, 'sell': sell.exchange, 'reason': reason}); continue
        ...
        if timed_out:
            ...
            for index, o in enumerate(opportunities, 1):
                ...
                verified = replace(o, metadata={**o.metadata, **transfer})
                verified = replace(verified, confidence=min(100.0, verified.confidence + (5.0 if network and contract else 0.0)))
