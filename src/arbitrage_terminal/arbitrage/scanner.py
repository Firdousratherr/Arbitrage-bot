...
class ArbitrageScanner:
    ...
    async def _one(self, name, adapter):
        ...
        try:
            ...
            diag.status = 'ok'; diag.retry_count = r1 + r2; diag.latency_ms = (time.perf_counter() - started) * 1000; diag.timestamp = datetime.now(timezone.utc).isoformat()
            breaker.success()
    ...
    async def scan(self, user_id, selected, filters, progress=None):
        ...
        async def get_transfer(exchange, asset):
            ...
            if remaining_scan <= 0:
                task.cancel(); transfer_tasks.pop(key, None); transfer_cache[key] = {}; return transfer_cache[key]
            try:
        ...
        if timed_out:
            ...
            if remaining_scan > 0 and keys:
                ...
                try: await asyncio.wait_for(asyncio.gather(*(preload(k) for k in keys)), timeout=remaining_scan)
                except asyncio.TimeoutError: warnings.append('Network validation reached the scan deadline; remaining routes were rejected as unverified.')
            validated = []
            for o in opportunities:
                if time.monotonic() - started_mono >= self.scan_deadline: timed_out = True; break
                buy = next(t for t in ticker_map[o.symbol] if t.exchange == o.buy_exchange and t.ask == o.buy_price)
                sell = next(t for t in ticker_map[o.symbol] if t.exchange == o.sell_exchange and t.bid == o.sell_price)
