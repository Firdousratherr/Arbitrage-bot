...
def pair_opportunity(buy,sell,buy_fee,sell_fee,withdrawal_cost_pct=0,max_age=10):
    ...
    bf=float(buy_fee or 0);sf=float(sell_fee or 0)
    net=gap-bf-sf-withdrawal_cost_pct;liq=min(buy.quote_volume,sell.quote_volume);pc=max(0,min(1,1-abs(gap)/10))
    score=confidence(freshness=age,liquidity=liq,price_consistency=pc,fees_known=fees_known,network_known=False,exchange_health=1)
    return Opportunity(buy.symbol,buy.exchange,sell.exchange,buy.ask,sell.bid,gap,buy_fee,sell_fee,withdrawal_cost_pct,net,buy.quote_volume,sell.quote_volume,age,score,MarketType.SPOT,{'network_available':False,'fee_data_available':fees_known,'deterministic':True})
