...
def confidence(*,freshness,liquidity,price_consistency,fees_known,network_known,exchange_health):
    age=max(0,min(1,1-freshness/30));liq=max(0,min(1,liquidity/100000));score=100*(.25*age+.20*liq+.20*price_consistency+.15*(1 if fees_known else .5)+.10*(1 if network_known else .5)+.10*exchange_health)
...
def withdrawal_cost_pct(buy_info, networks, base_amount):
    ...
    if base_amount is None or not isfinite(float(base_amount)) or float(base_amount)<=0:return None
    by_network=_networks(buy_info);known=[]
    for network in networks:
        value=by_network.get(str(network).upper(),{}).get('fee')
        try:fee=float(value)
        except (TypeError,ValueError):continue
        if isfinite(fee) and fee>=0:known.append(fee)
...
def pair_opportunity(buy,sell,buy_fee,sell_fee,withdrawal_cost_pct=None,max_age=10,transfer=None):
    ...
    buy_identity=getattr(buy,'asset_identity',None);sell_identity=getattr(sell,'asset_identity',None)
    identity_conflict=bool(buy_identity and sell_identity and buy_identity!=sell_identity)
    ...
    net=None if not fees_known else gap-float(buy_fee)-float(sell_fee)-float(withdrawal_cost_pct or 0)
    ...
    meta={'network_available':network_known,'contract_match':contract_match,'compatible_networks':(transfer or {}).get('networks',[]),'fee_data_available':fees_known,'withdrawal_fee_available':withdrawal_cost_pct is not None,'deterministic':True,'transfer_verified':network_known and contract_match,'asset_identity_verified':bool(buy_identity and sell_identity),'asset_identity_conflict':identity_conflict}
