...
class Repository:
    ...
    async def set_ai_mode(self, user_id, mode):
        await self.db.execute('UPDATE user_ai_config SET result_mode=? WHERE user_id=?', (mode, user_id))
        await self.db.commit()
    ...
    def filters_from_row(self, row):
        try:
            ...
            raw = {}
        merged = {**DEFAULT_FILTERS, **raw}
        merged['selected_coins'] = set(merged.get('selected_coins') or [])
    ...
    async def vip_active(self, user_id):
        ...
        if r['vip_expiry'] and r['vip_expiry'] <= datetime.now(timezone.utc).isoformat():
            await self.db.execute("UPDATE users SET vip_status='expired' WHERE telegram_id=?", (user_id,))
            await self.db.commit()
            return False
        return True
