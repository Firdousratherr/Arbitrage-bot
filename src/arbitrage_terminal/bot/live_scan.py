...
def _progress_percent(stage,data,selected):
    ...
    if stage=='exchange':
        total=max(1,int(data.get('total',selected) or selected or 1)); completed=max(0,min(total,int(data.get('completed',0) or 0))); return 10+int(35*completed/total)
    if stage=='markets':return 50
    if stage=='fees':return 65
    if stage=='candidates':return 80
    if stage=='orderbook':
        total=max(1,int(data.get('total',1) or 1)); validated=max(0,min(total,int(data.get('validated',0) or 0))); return 80+int(10*validated/total)
    if stage in {'recovery','exchange_recovery'}:return max(10,min(90,int(data.get('percent',50) or 50)))
    if stage=='network':
        total=max(1,int(data.get('total',1) or 1)); validated=max(0,min(total,int(data.get('validated',0) or 0))); return 90+int(5*validated/total)
    if stage=='opportunity':return 95
    if stage=='complete':return 100
    return 50

def _progress_bar(percent):
    filled=max(0,min(10,int(percent/10))); return '█'*filled+'░'*(10-filled)
...
def _window(stage,selected,states,comparisons=0,opportunities=0,best=None,detail='',frame=0,percent=None,elapsed=None):
    ...
    for name,status in states.items():
        icon='🟢' if status=='healthy' else '🔴' if status in {'failed','quarantined'} else '🟡'; lines.append(f'{icon} {html.escape(name.title())} · {html.escape(status)}')
    lines += ['',f'🔄 <b>Comparisons:</b> {comparisons:,}',f'🔥 <b>Opportunities:</b> {opportunities:,}']
    if best:lines += ['',f'💎 <b>Best so far:</b> {html.escape(best["symbol"])} +{best["gap"]:.3f}%',f'   🟢 {html.escape(best["buy"])} → 🔴 {html.escape(best["sell"])}']
    if detail:lines += ['',f'ℹ️ {html.escape(detail)}']
    return '\n'.join(lines)

async def live_scan_callback(update,context):
    q=update.callback_query; svc=context.application.bot_data['service']; uid=q.from_user.id; await q.answer('🚀 Scan started')
    ...
    try:import json; selected=json.loads(row['exchanges'] or '[]')
    ...
    async def heartbeat():
        nonlocal frame,current_stage,current_percent,current_detail
        while True:
            await asyncio.sleep(5); frame+=1; elapsed=time.monotonic()-started
            recovering=[]
            for name in selected:
                adapter=context.application.bot_data['exchanges'].get(name)
                if adapter is None:continue
                try:
                    health=adapter.health_snapshot()
                    state=str(health.get('state','healthy'))
                    if state in {'recovering','quarantined'}:recovering.append((name,state))
                    if state=='recovering':states[name]='recovering'
                    elif state=='quarantined':states[name]='quarantined'
                except Exception:pass
            detail=current_detail or f'No new stage update · {elapsed:.0f}s elapsed'
            if recovering:
                current_stage='🛠️ Exchange self-healing in progress…'; current_percent=max(current_percent,50); detail=' · '.join(f'{n.title()}: {s}' for n,s in recovering)
            await safe_edit(_window(current_stage,len(selected),states,current_comparisons,current_opportunities,current_best,detail,frame,current_percent,elapsed))
    ...
    async def progress(stage,data):
        ...
        labels={'start':'🔌 Connecting to exchanges…','exchange':'📡 Exchange response received','markets':'📊 Markets loaded · building comparison set…','fees':'💸 Fee data loaded · evaluating gaps…','candidates':'🔎 Comparing prices before order-book validation…','orderbook':'📚 Validating executable order books…','network':'🌐 Validating transfer networks…','recovery':'🛠️ Exchange error detected · fixing automatically…','exchange_recovery':'🛠️ Exchange recovery in progress…','opportunity':'💎 Live opportunity found!','complete':'✅ Scan complete'}
    ...
    try:
        snap=await svc.run_scan(uid,progress=progress); p=snap.to_dict(); scan_id=p['scan_id']; rows=[[('🔥 Best Results',f'page:{scan_id}:0:best'),('📋 All Results',f'page:{scan_id}:0:all')],[('📡 Diagnostics',f'rdiag:{scan_id}:0:all'),('🔎 Debug Coin',f'rdebug:{scan_id}:0:all')],[('🧠 AI Analysis',f'raian:{scan_id}:0:all'),('🔄 Scan Again','scan')],[('💬 Arbitrage Chat','chat:open'),('👨‍💻 Contact Developer','contact:open')]]
        if uid in svc.settings.admin_ids:rows.append([('🛠️ Fix Exchanges','repair:open')])
        await q.edit_message_text(scan_status(p),parse_mode='HTML',reply_markup=kb(rows))
    except Exception as exc:
        rows=[[('🔄 Try Again','scan'),('🏠 Dashboard','home')],[('💬 Arbitrage Chat','chat:open')]]
        if uid in svc.settings.admin_ids:rows.append([('🛠️ Fix Exchanges','repair:open')])
        await q.edit_message_text(f'❌ <b>SCAN FAILED</b>\n\n{type(exc).__name__}: {html.escape(str(exc)[:250])}',parse_mode='HTML',reply_markup=kb(rows))
    finally:
