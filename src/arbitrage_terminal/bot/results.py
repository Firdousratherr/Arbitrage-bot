...
def _items_for_mode(items, mode):
    if mode == 'best':
        return sorted(
            items,
            key=lambda o: (
                float(o.get('estimated_net_profit') or 0),
                float(o.get('raw_gap') or 0),
            ),
            reverse=True,
        )
    return list(items)
...
def _page_markup(scan_id, page, total, mode, items):
    ...
    if nav:
        rows.append(nav)
    rows.append([
        ('🔥 Best', f'page:{scan_id}:0:best'),
        ('📋 All', f'page:{scan_id}:0:all'),
    ])
    rows.append([
        ('📡 Diagnostics', f'rdiag:{scan_id}:{page}:{mode}'),
        ('🔎 Debug Coin', f'rdebug:{scan_id}:{page}:{mode}'),
    ])
    rows.append([
        ('🧠 AI Analysis', f'raian:{scan_id}:{page}:{mode}'),
        ('🏠 Dashboard', 'home'),
    ])
    return kb(rows)
...
def _empty_text(p, validation_mode):
    ...
    txt = (
        ...
        f'🏦 {len(p.get("healthy_exchanges", []))} healthy · '
        f'{len(p.get("failed_exchanges", []))} failed\n'
    ...
    try:
        ...
        if not p:
            await q.edit_message_text(
                '⚠️ Scan snapshot not found.',
                reply_markup=kb([[('🏠 Dashboard', 'home'), ('🔄 Scan Again', 'scan')]]),
            )
            return
        ...
        await q.edit_message_text(
            txt,
            parse_mode='HTML',
            reply_markup=_page_markup(scan_id, page, total, mode, current),
        )
    except Exception as exc:
        await q.edit_message_text(
            f'⚠️ <b>Results error</b>\n\n{type(exc).__name__}: {html.escape(str(exc)[:220])}',
            parse_mode='HTML',
            reply_markup=kb([[('🏠 Dashboard', 'home'), ('🔄 Scan Again', 'scan')]]),
        )
...
def _order_text(route):
    ...
    return (
        ...
        f"{levels(route['buy'], 'asks')}\n\n"
        f"🔴 <b>SELL on {html.escape(route['sell_exchange'])}</b> — required <b>BIDS</b>\n"
        f"{levels(route['sell'], 'bids')}\n\n"
        'ℹ️ Only the order-book sides required for this arbitrage route are shown. '
        'Prices are live market data, not placed orders.'
    )
...
async def results_detail_callback(update, context):
    ...
    try:
        ...
        p = await svc.scan(uid, scan_id)
        if not p:
            await q.edit_message_text('⚠️ Scan snapshot not found.', reply_markup=kb([[('🏠 Dashboard', 'home')]]))
            return
        ...
        if kind == 'rorder':
            ...
            await q.edit_message_text(
                _order_text(route), parse_mode='HTML', reply_markup=_detail_back(scan_id, page, mode)
            )
            return
        ...
        if kind == 'raian':
            ...
            await q.edit_message_text(
                '🧠 <b>AI SCAN ANALYSIS</b>\n\n' + html.escape(str(text)),
                parse_mode='HTML',
                reply_markup=_detail_back(scan_id, page, mode),
            )
            return
