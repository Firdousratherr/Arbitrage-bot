...
async def contact_open(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Open a private developer contact form and initialize its state."""
...
async def contact_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Process one contact-form text step. Returns True when consumed."""
...
async def contact_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle contact form buttons. Conversation wrapper decides whether to end."""
    q = update.callback_query
    await q.answer()
    data = q.data

    if data == 'contact:open':
        return await contact_open(update, context)
    ...
    if data == 'contact:send':
        ...
        if any(not str(form.get(key, '')).strip() for key in required):
            ...
            await q.edit_message_text(
                '⚠️ The contact form was incomplete. Please enter your full name to start again.',
        ...
        if sent:
            await q.edit_message_text(
                '✅ <b>Message sent.</b>\n\nYour request has been delivered to the developer privately.',
                parse_mode='HTML',
                reply_markup=kb([[('🏠 Dashboard', 'home')]]),
            )
        else:
            await q.edit_message_text(
                '⚠️ The developer inbox is temporarily unavailable. Please try again later.',
                reply_markup=kb([[('🏠 Dashboard', 'home')]]),
            )
        return ConversationHandler.END
...
async def contact_state_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    consumed = await contact_text(update, context)
    if not consumed:
        return ConversationHandler.END
    return CONTACT_FORM


async def contact_state_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    return await contact_callback(update, context)
