...
async def start(update, context):
    ...
    await svc.ensure_user(update.effective_user)
    row = await svc.get_user(update.effective_user.id)
    if not row['email']:
        context.user_data['await_email'] = True
        await update.effective_message.reply_text('⚡ <b>Welcome to Arbitrage Terminal</b>\n\nSend your email to continue.', parse_mode='HTML')
        return
    ...
    await update.effective_message.reply_text(
        dashboard(row), parse_mode='HTML',
        reply_markup=kb([[('🔎 Scan Arbitrage','scan'),('🏦 Exchanges','exchanges')], [('📊 Filters','filters'),('🧠 AI','ai')], [('📡 Status','status'),('📋 History','history')], [('⚙️ Settings','settings'),('❓ Help','help')], [('👨‍💻 Contact Developer','contact:open')]])
    )
...
async def exchange_markup(context, row):
    ...
    rows += [[('Select All','ex:all'),('Clear All','ex:none')], [('✅ Save Selection','ex:save')]]
    return kb(rows)
...
async def genkey(update, context):
    if update.effective_user.id not in context.application.bot_data['settings'].admin_ids:
        await update.effective_message.reply_text('Not authorized.'); return
    if len(context.args) != 2:
        await update.effective_message.reply_text('Usage: /genkey KEY DAYS|lifetime'); return
    try:
        key = await context.application.bot_data['repo'].create_vip_key(update.effective_user.id, context.args[0], context.args[1])
        await update.effective_message.reply_text(f'🔑 Created VIP key: <code>{key}</code>', parse_mode='HTML')
...
async def callbacks(update, context):
    ...
    if data == 'vip:enter':
        context.user_data['await_vip_key'] = True; await q.edit_message_text('🔐 <b>VIP KEY</b>\n\nSend your VIP key as a message.', parse_mode='HTML')
    elif data == 'home': await start(update, context)
    elif data.startswith('aim:'):
        await context.application.bot_data['service'].set_ai_mode(q.from_user.id, data.split(':', 1)[1]); await q.edit_message_text('✅ AI result mode saved.')
