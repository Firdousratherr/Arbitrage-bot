...
def kb(rows):
    ...
    for row in rows:
        if not row:
            continue
        buttons.append([InlineKeyboardButton(t,url=d) if str(d).startswith(('https://','tg://')) else InlineKeyboardButton(t,callback_data=d) for t,d in row])
...
async def start(update,context):
    ...
    await update.effective_message.reply_text(dashboard(row),parse_mode='HTML',reply_markup=kb([[('🔎 Scan Arbitrage','scan'),('🏦 Exchanges','exchanges')],[('📊 Filters','filters'),('🧠 AI','ai')],[('📡 Status','status'),('📋 History','history')],[('⚙️ Settings','settings'),('❓ Help','help')],[('👨‍💻 Contact Developer','https://t.me/firdousratherr')]]))
...
async def text(update,context):
    if context.user_data.pop('await_email',False):
        ...
        await context.application.bot_data['service'].ensure_user(update.effective_user,email);row=await context.application.bot_data['service'].get_user(update.effective_user.id)
        if context.application.bot_data['settings'].require_vip and not await context.application.bot_data['repo'].vip_active(update.effective_user.id):
            txt,markup=vip_prompt();await update.effective_message.reply_text('Email saved.\n\n'+txt,parse_mode='HTML',reply_markup=markup);return
    ...
    if context.user_data.pop('await_vip_key',False):
        ...
        ok,msg=await context.application.bot_data['repo'].redeem_vip_key(update.effective_user.id,key)
        if not ok:
            context.user_data['await_vip_key']=True;await update.effective_message.reply_text('⚠️ '+msg+'\n\nSend another VIP key to try again.');return
...
async def vipkey(update,context):
    ...
    if ok:
        context.user_data.pop('await_vip_key',None);row=await context.application.bot_data['service'].get_user(update.effective_user.id);await update.effective_message.reply_text('🔐 '+msg+'\n\nChoose your exchanges.',reply_markup=await exchange_markup(context,row))
...
async def callbacks(update,context):
    ...
    if data=='vip:enter':context.user_data['await_vip_key']=True;await q.edit_message_text('🔐 <b>VIP KEY</b>\n\nSend your VIP key as a message.',parse_mode='HTML');return
    if data=='scan':
        row=await svc.get_user(uid)
        if svc.settings.require_vip and not await svc.repo.vip_active(uid):await q.edit_message_text('🔒 <b>Active VIP access is required.</b>',parse_mode='HTML',reply_markup=kb([[('🔑 Enter VIP Key','vip:enter')]]));return
        await q.edit_message_text('⚡ Starting scan...\n🔄 Connecting to exchanges...');await asyncio.sleep(.1);await q.edit_message_text('🔍 Scanning markets...\n📊 Comparing markets...');snap=await svc.run_scan(uid);p=snap.to_dict();await q.edit_message_text(scan_status(p),parse_mode='HTML',reply_markup=kb([[('🔥 Best Results',f'page:{p["scan_id"]}:0'),('📋 All Results',f'page:{p["scan_id"]}:0')],[('📡 Diagnostics',f'diag:{p["scan_id"]}'),('🔎 Debug Coin',f'debug:{p["scan_id"]}')],[('🧠 AI Analysis',f'aian:{p["scan_id"]}'),('🔄 Scan Again','scan')],[('👨‍💻 Contact Developer','https://t.me/firdousratherr')]]));return
    if data.startswith('ex:'):return
    if data=='help':await q.edit_message_text('<b>COMMANDS</b>\n/scan /results /exchanges /filters /settings /status /diagnostics /ai /aiprobe /aiprobestatus /aiprobelogs /aiproberepair /vipkey\n\n👨‍💻 <b>Developer Support</b>\nFor help, bugs or suggestions: @firdousratherr',parse_mode='HTML',reply_markup=kb([[('👨‍💻 Contact Developer','https://t.me/firdousratherr')],[('🏠 Dashboard','home')]]));return
    if data=='filters':
    ...
    if data.startswith('val:'):
        mode=data.split(':',1)[1]
        if mode not in {'strict','loose'}:await q.edit_message_text('⚠️ Invalid validation mode.');return
