...
class TerminalService:
    def __init__(self,repo,scanner,ai,settings,exchanges=None):
        self.repo=repo; self.scanner=scanner; self.ai=ai; self.settings=settings; self.exchanges=exchanges or {}
    ...
    async def run_scan(self,user_id):
        row=await self.repo.user(user_id); selected=json.loads(row['exchanges'] or '[]')
        snap=await self.scanner.scan(user_id,selected,self.repo.filters_from_row(row)); await self.repo.save_scan(snap); return snap
    ...
    async def ai_scan_analysis(self,user_id,snap):
        row=await self.repo.user(user_id); mode=AIMode(row['result_mode'] or 'off')
        payload={'scan_id':snap['scan_id'],'opportunities':snap['opportunities'],'healthy_exchanges':snap['healthy_exchanges'],'failed_exchanges':snap['failed_exchanges'],'warnings':snap['warnings']}
        return await self.ai.analyze(mode,'Analyze only supplied deterministic scan data. Never invent market facts.',payload)
    async def order_route(self,user_id,scan_id,index):
        """Fetch only the two books needed for the selected opportunity: buy asks and sell bids."""
        ...
        buy_book,sell_book=await __import__('asyncio').gather(
            buy.get_orderbook(o['symbol'],limit=10), sell.get_orderbook(o['symbol'],limit=10), return_exceptions=True)
