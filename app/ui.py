...
def format_scan_count(count: int) -> str:
    ...
    if diagnostics:
        ...
        visible = min(8, len(diagnostics))
        for item in diagnostics[:visible]:
            symbol = _escape_html(str(item.get("symbol", "unknown")))
            gaps = item.get("gaps", {}) or {}
            gap_text = "; ".join(
                f"{_escape_html(str(exchange))}: {_escape_html(str(reason))[:100]}"
                for exchange, reason in gaps.items()
            )
            lines.append(f"• <b>{symbol}</b> — {gap_text}")
        if len(diagnostics) > visible:
            lines.append(f"• … and {len(diagnostics) - visible} more symbols with data gaps")
    return "\n".join(lines)
...
def format_opportunity_card(
    ...
    # Backward-compatible legacy calling conventions:
    # format_opportunity_card(..., title="...") and (..., "title text").
    ...
    if tag is None:
        if card_number is not None:
            ...
            rank_label = rank[number - 1] if 1 <= number <= 10 else f"#{number}"
            tag = f"{rank_label} 🔍 SCAN RESULT"
    ...
    buy_transfer = metadata.get("buy_transfer", {}) or {}
    sell_transfer = metadata.get("sell_transfer", {}) or {}
    if transfer_verify == "loose_mode":
        ...
        buy_networks = [n.get("network") for n in buy_transfer.get("networks", []) if n.get("deposit")]
        sell_networks = [n.get("network") for n in sell_transfer.get("networks", []) if n.get("withdraw")]
        transfer_line = (
            f"✅ Verified route • {matching}"
            if matching and buy_networks and sell_networks
            else "⏳ Transfer verification pending"
        )
    ...
    if trade_size and trade_size > 0:
        ...
        gross_text = f"${_compact_number(gross, 4)}"
        fees_text = f"${_compact_number(fees, 4)}"
        net_text = f"${_compact_number(net, 4)}"
        size_text = f"${_compact_number(trade_size, 4)}"
    else:
        gross_text = f"{opportunity.raw_spread:.2f}%"
        fees_text = f"{opportunity.raw_spread - opportunity.net_profit:.2f}%"
        net_text = f"{opportunity.net_profit:.2f}%"
        size_text = "$1,000"

    coin = opportunity.symbol.split("/")[0]
    coin_amount = (
        trade_size / opportunity.buy_price
        if trade_size and opportunity.buy_price > 0
        else 1000 / opportunity.buy_price
    ...
    lines.extend(
        [
            "🟢 <b>BUY HERE</b>",
            f"   🌐 {_escape_html(opportunity.buy_exchange)}  •  <b>${_compact_number(opportunity.buy_price, 8)}</b>",
            "",
            "🔴 <b>SELL HERE</b>",
            f"   🌐 {_escape_html(opportunity.sell_exchange)}  •  <b>${_compact_number(opportunity.sell_price, 8)}</b>",
            "",
            SECTION_SEPARATOR,
            ...
            f"   💸 Fees        − {fees_text}",
            f"   🚀 <b>NET        {net_text}</b>",
            f"   🎯 Spread      <b>{opportunity.net_profit:.2f}%</b>",
            "",
            "📋 <b>TRADE DETAILS</b>",
            f"   💵 Size        {size_text}",
            f"   🪙 Amount      {_compact_number(coin_amount, 6)} {coin}",
            f"   🛡️ Transfer    {transfer_line}",
            BOTTOM,
        ]
    )
    # Compact Telegram card: deliberately no blank lines inside the result.
    return "\n".join(lines).replace("\n\n", "\n")
...
def opportunity_buttons(identifier: str) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        [
            [InlineKeyboardButton("📖  Order Book & Analysis", callback_data=f"details:{identifier}")],
...
def format_background_alert(opportunity, identifier: str) -> str:
    return format_opportunity_card(opportunity, identifier)
...
def format_scan_summary(
    ...
    *,
    ...
    matching_selected: int,
    results_shown: int,
) -> str:
    lines = [
        "🔍 <b>SCAN COMPLETE</b>",
        "",
        f"✨ {len(opportunities)} opportunities ready to review." if opportunities else "📭 No opportunities found",
        f"🌐 {exchange_count} exchanges checked.",
        f"📊 {opportunities_found} found • {matching_selected} matched filters • {results_shown} shown.",
    ]
    if opportunities:
        lines.extend(
            ["", "🏆 <b>TOP OPPORTUNITIES</b>"]
            + [
                f"• {_escape_html(str(getattr(opportunity, 'symbol', 'unknown')))} — {getattr(opportunity, 'net_profit', 0):.2f}% net"
                for opportunity in list(opportunities)[:3]
            ]
        )
    return "\n".join(lines)
...
def format_order_book(levels: Iterable[Sequence[float]], *, title: str) -> str:
    rows = [
        f"{float(level[0]):.8f} × {float(level[1]):.6f}"
...
def format_opportunity_details(
    row: dict,
    buy_fill: float,
    sell_fill: float,
    buy_fee: float,
    ...
    sell_slippage: float,
    transfer_text: str,
    buy_book: Sequence[Sequence[float]],
    sell_book: Sequence[Sequence[float]],
) -> str:
    lines = _panel(
        "📖 DETAILS • ORDER BOOK",
        f"<b>{_escape_html(row['symbol'])}</b> • execution analysis",
    )
    lines.extend(
        [
            ...
            "",
            SECTION_SEPARATOR,
            "🧮 <b>EXECUTION ANALYSIS</b>",
            f"   💵 Gross profit  ${_compact_number(gross_profit, 4)}",
            f"   💸 Buy fee       {buy_fee * 100:.4f}%",
            f"   💸 Sell fee      {sell_fee * 100:.4f}%",
            f"   🚀 <b>Net profit    ${_compact_number(net_profit, 4)}</b>",
            f"   📉 Buy slippage  {buy_slippage:.2f}%",
            f"   📈 Sell slippage {sell_slippage:.2f}%",
            f"   💧 Buy volume    {_compact_number(row['volume_buy'])}",
            f"   💧 Sell volume   {_compact_number(row['volume_sell'])}",
            "",
            SECTION_SEPARATOR,
            "🛡️ <b>TRANSFER STATUS</b>",
...
def format_paper_trade(
    ...
    *,
    ...
    lines.extend(
        [
            f"🪙 <b>{_escape_html(opportunity.symbol)}</b>",
            f"🟢 {_escape_html(opportunity.buy_exchange)}  ${_compact_number(buy_price, 8)}",
            "        ↓  simulated route",
            f"🔴 {_escape_html(opportunity.sell_exchange)}  ${_compact_number(sell_price, 8)}",
            "",
            SECTION_SEPARATOR,
            f"💵 Position size  ${_compact_number(size, 6)}",
            f"📈 Gross result   ${_compact_number(expected_gross, 6)}",
            f"{icon} <b>Net P/L         ${_compact_number(estimated_net, 6)}</b>",
            BOTTOM,
        ]
    )
    return "\n".join(lines)
...
def format_status_message(
    ...
    lines.extend(
        [
            ...
            f"⏯ <b>Alerts</b>    {'PAUSED' if paused else 'LIVE'}",
            f"⚠️ <b>Loose mode</b> {'ON' if loose_mode else 'OFF'}",
            "",
            SECTION_SEPARATOR,
            ...
            f"⏱️ Cooldown     {filters.get('alert_cooldown', 300)}s",
            BOTTOM,
        ]
    )
    return "\n".join(lines)
...
def format_filters_message(filters: dict) -> str:
    ...
    lines.extend(
        [
            ...
            "/setminvolume, /watchlist, /blacklist and /setalertfreq.",
            BOTTOM,
        ]
    )
    return "\n".join(lines)
...
def format_portfolio(user_trades: list, total_balance: float, vip_limit: float) -> str:
    total_pnl = sum(trade.get("profit", 0) for trade in user_trades)
    icon = "🟢" if total_pnl >= 0 else "🔴"
    lines = _panel("📊 YOUR PORTFOLIO", "Your simulated trading dashboard")
    lines.extend(
        [
            f"💰 <b>Simulated Balance</b>  ${_compact_number(total_balance, 4)}",
            f"{icon} <b>Total P/L</b>          ${_compact_number(total_pnl, 4)}",
            f"📈 <b>Trades</b>             {len(user_trades)}",
            f"🎯 <b>VIP limit</b>          ${_compact_number(vip_limit, 4)}",
            "",
            SECTION_SEPARATOR,
        ]
    )
    if user_trades:
        ...
        for trade in user_trades[:5]:
            ...
            icon = "🟢" if pnl >= 0 else "🔴"
            date = str(trade.get("created_at", ""))[:10] or "unknown"
            lines.append(
                f"{icon} {symbol} • ${_compact_number(trade.get('size', 0), 4)} • {date} • P/L ${_compact_number(pnl, 4)}"
            )
