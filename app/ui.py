...
def format_scan_count(count: int) -> str:
    ...
    snapshot = get_last_scan_snapshot()
...
def format_opportunity_card(opportunity, identifier: str, card_number: int | str | None = None, tag: str | None = None, trade_size: float | None = None, title: str | None = None) -> str:
    ...
    if tag is None:
        if card_number is not None:
            ...
            n = int(card_number)
            tag = f"{ranks[n - 1] if n <= 10 else f'#{n}'} 🔍 SCAN RESULT"
    ...
    tv = metadata.get("transfer_verification")
    bt = metadata.get("buy_transfer", {})
    st = metadata.get("sell_transfer", {})
    if tv == "loose_mode":
        transfer_line = "⚠️ Transfer checks skipped — verify manually"
    elif tv == "not_verified":
        transfer_line = "🛡️ Unverified — manual check recommended"
    else:
        matching = metadata.get("matching_network", "")
        bn = [n.get("network") for n in bt.get("networks", []) if n.get("deposit")]
        sn = [n.get("network") for n in st.get("networks", []) if n.get("withdraw")]
        transfer_line = f"✅ Verified route • {matching}" if matching and bn and sn else "⏳ Transfer verification pending"
...
def format_scan_summary(opportunities: Sequence[object], *, exchange_count: int, opportunities_found: int, matching_selected: int, results_shown: int) -> str:
    ...
    if opportunities:
        lines.extend([
            ...
            *[
                f"{['1️⃣', '2️⃣', '3️⃣'][i - 1] if i <= 3 else f'{i}.'} {_escape_html(str(getattr(o, 'symbol', 'unknown')))} — {getattr(o, 'net_profit', 0):.2f}% net"
                for i, o in enumerate(list(opportunities)[:3], 1)
            ],
        ])
    return "\n".join(lines)
...
def format_order_book(levels: Iterable[Sequence[float]], *, title: str) -> str:
    rows = [f"{float(l[0]):.8f} × {float(l[1]):.6f}" for l in list(levels)[:5] if len(l) >= 2]
...
def format_opportunity_details(row: dict, buy_fill: float, sell_fill: float, buy_fee: float, sell_fee: float, gross_profit: float, net_profit: float, buy_slippage: float, sell_slippage: float, transfer_text: str, buy_book: Sequence[Sequence[float]], sell_book: Sequence[Sequence[float]]) -> str:
    lines = _panel("📖 DETAILS • ORDER BOOK", f"<b>{_escape_html(row['symbol'])}</b> • execution analysis")
    lines.extend([
